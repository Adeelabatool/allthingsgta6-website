...
function HypeCalculator() {
  ...
  const score = Math.min(
    100,
    Math.round(
      trailers * 4 + hours * 1.2 + preorder * 0.3 + reddit * 6 + played * 5
    )
  );

  const tier =
    score >= 90 ? { label: "🔥 Rockstar would hire you", color: "chip-hot" }
    : score >= 70 ? { label: "🚨 Maximum Hype", color: "chip-hot" }
    : score >= 50 ? { label: "📈 Hyped Citizen", color: "chip-neon" }
    : score >= 30 ? { label: "🕶 Casual Observer", color: "chip-neon" }
    ...
    <SiteShell>
      <div className="container-page py-12 max-w-3xl">
        ...
        <div className="mt-8 surface p-6 space-y-5">
          <Slider label="Trailer rewatches" value={trailers} setValue={setTrailers} max={20} unit="times" />
          <Slider label="Hours you'd play at launch / week" value={hours} setValue={setHours} max={40} unit="hrs" />
          <Slider label="Max $ you'd spend on pre-order" value={preorder} setValue={setPreorder} max={250} unit="$" />
          <Slider label="GTA subreddit visits / day" value={reddit} setValue={setReddit} max={20} unit="visits" />
          <Slider label="Previous GTA games played" value={played} setValue={setPlayed} max={8} unit="games" />
        </div>

        <div className="mt-6 surface p-8 text-center">
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Your Hype Score</div>
          <div className="heading-display text-7xl mt-2 tabular-nums">{score}</div>
          <div className="mt-3"><span className={`chip ${tier.color}`}>{tier.label}</span></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-accent tabular-nums">{value} {unit}</span>
      </div>
      <input
        type="range"
        min={0}
