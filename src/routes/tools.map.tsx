...
const POINTS = [
  {
    ...
    y: 70,
    label: "Ocean Beach",
    info: "Vice City's iconic art-deco beachfront strip.",
  },
  { id: "downtown", x: 40, y: 50, label: "Downtown", info: "High-rise financial core." },
  {
    ...
    y: 45,
    label: "Little Haiti",
    info: "Cultural neighborhood district.",
  },
  {
    id: "port",
    x: 25,
    ...
    info: "Cargo, smuggling, criminal opportunity.",
  },
  {
    id: "everglades",
    x: 70,
    y: 65,
    label: "Everglades",
    info: "Swamp biome — wildlife & smuggling.",
  },
    ...
    <SiteShell>
      <div className="container-page py-12">
        ...
        <p className="mt-2 text-muted-foreground">
          Click landmarks to inspect. Geometry is speculative pending official release.
        </p>
          ...
          <div className="surface p-3">
            <div className="relative w-full aspect-[4/3] rounded-md overflow-hidden bg-gradient-to-br from-[oklch(0.2_0.05_270)] via-[oklch(0.25_0.08_330)] to-[oklch(0.3_0.1_200)]">
              ...
              <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full opacity-40">
                <path
                  d="M10 30 Q 30 20 50 25 T 90 35 L 95 70 Q 70 80 50 75 T 10 80 Z"
                  fill="oklch(0.35 0.1 270)"
                />
              </svg>
              {POINTS.map((p) => (
                <button
                ...
                >
                  <span
                    className={`block w-3 h-3 rounded-full border-2 border-background ${active.id === p.id ? "bg-primary scale-150" : "bg-accent"} transition-all`}
                  />
            ...
            <ul className="mt-2 space-y-1">
              {POINTS.map((p) => (
                <li key={p.id}>
                  <button
                    onClick={() => setActive(p)}
                    className={`text-left w-full px-2 py-1 rounded ${active.id === p.id ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {p.label}
                  </button>
                </li>
